...
#define Q_value_init -10000



uint8_t Q_table[(N_lim_ang-1)*(N_lim_gir-1)][N_actions];
...
uint8_t lim_ang[N_lim_ang];  
const int lim_gir[N_lim_gir] = {-1000, 1000};             
...
void setup() {
  ...
  //INICIALIZACIÓN VECTOR CON LIMITES DE ÁNGULOS
  float delta_angle = (175)/(N_lim_ang-1);
  for (size_t j = 0; j<N_lim_ang; j++){
    lim_ang[j] = delta_angle*j; 
  }
  /*for (size_t j = 0; j<N_lim_ang; j++){
    Serial.println(lim_ang[j]); 
  }*/
...
void inicializartabla(){
  for (size_t i = 0; i<(N_lim_ang-1)*(N_lim_gir-1); i++)
  {
    ...
    {
      Q_table[i][j] = Q_value_init;     //Se fija un valor bajo para evitar coger como máximo un estado que nunca se haya entrado
    }
  }
}
...
void enviardatos(){
  for (size_t i = 0; i<(N_lim_ang-1)*(N_lim_gir-1); i++)
  {
    for (size_t j = 0; j<N_actions; j++)
    {
      Serial.println(Q_table[i][j],4);
      delay(5);
    }
  }
  for (size_t i = 0; i<N_training_episodes; i++){
    Serial.println(v_reward[i],4);
    delay(5);
  }
  for (size_t i = 0; i<N_training_episodes; i++){
...
void cargartabla(){
  ...
  while (t){
    while (Serial.available()) {
      ...
      if (inChar == ',') {  
        Q_table[fila][columna] = (int)(inputString.toFloat());  // Convertir a float
        ...
        if (inputString.length() > 0) {
          Q_table[fila][columna] = (int)(inputString.toFloat());
          inputString = "";
        }
        fila++;
        columna = 0;
...
void learn(){
  ...
  for (episode = episodio_init; episode < N_training_episodes; episode++){
    ...
    for (step; step < max_steps; step++){
      ...
      while (deltaT<3000+delay_time*1000){
        currentTime = micros();
      ...
      uint8_t new_state = mapeo(angle, 0);
      float reward = deltaT*1e-3*calculate_reward(new_state);
      s_reward += reward;
      /*uint8_t best_action = greedy_policy(new_state);
      float Q_value_new_state = Q_table[new_state][best_action];
      
      if (Q_table[state][action] == Q_value_init){
        Q_table[state][action] = 0;
      }
      if (Q_value_new_state == Q_value_init){
        Q_value_new_state = 0;
      }
      
      Q_table[state][action] = Q_table[state][action] + learning_rate * (reward + gamma*Q_value_new_state - Q_table[state][action]);
*/
      if (terminated){
...
inline float calculate_reward(uint8_t newState){
  ...
  if (newState >= 16  && newState <= 17){
    reward = 5;
  }
  else if (newState == 15 || newState == 18){
    reward = 2;
  }
  else if (newState > 25  || newState < 8){
    reward = -20;
